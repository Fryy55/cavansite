...
public:
	...
	constexpr BreezeTheme(
		...
		if (list.size() != 3z) {
			aurora::log::warn(
				"[cavansite] Saved theme setting is corrupted. Using default theme"
			);
		...
		if (!this->validate()) {
			aurora::log::warn(
				"[cavansite] Saved theme setting is invalid in the current configuration. Using default theme"
			);
...
private:
	...
	[[nodiscard]] constexpr bool validate() const noexcept {
		...
		if (auto mode = std::to_underlying(m_providedMode); 2u < mode)
			return false;
...
public:
	...
	constexpr operator QVariant() noexcept {
		return {
...
public:
	[[nodiscard]] constexpr QString getPrefix() const {
	...
	[[nodiscard]] constexpr QColor getAccentColor() const {
		switch (m_color) {
			#ifdef CAVANSITE_BREEZE_THEME_RED
				case Color::Red:
					return { 184, 50, 50 };
	...
	constexpr void setMode(Mode providedMode) noexcept {
		m_providedMode = providedMode;
		this->inferRealMode();
